...
if ( ! function_exists( 'sparch_posted_on' ) ) :
	/**
...
if ( ! function_exists( 'sparch_posted_by' ) ) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function sparch_posted_by() {
		global $post;
		$byline = sprintf(
			/* translators: %s: post author. */
			esc_html_x( '%s', 'post author', 'sparch' ),
...
if ( ! function_exists( 'sparch_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function sparch_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'sparch' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'sparch' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'sparch' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'sparch' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'sparch' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'sparch' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'sparch_entry_meta_footer' ) ) :
		/**
	 * Prints HTML with meta information for the categories, tags and comments.
...
if ( ! function_exists( 'sparch_post_thumbnail' ) ) :
	/**
	 ...
	 */
	function sparch_post_thumbnail() {
...
function sparch_is_comment_by_post_author( $comment = null ) {
 ...
 */
 ...
 if(!function_exists('sparch_logo')) {
	...
	*/
		...
		function sparch_logo () {
			...
			if(class_exists( 'Redux' ) ) {
				...
				$logo_sticky = sparch_options('sparch_sticky_logo'); 
				$logo_ratina = sparch_options('sparch_retina_logo');
				$logo_ratina_sticky = sparch_options('sparch_retina_sticky_logo');
  ...
  * display tag 
  */
 if(!function_exists('sparch_single_page_tag'))  {
	 function sparch_single_page_tag() {
		 if(has_tag() ) :
		 ?>
		 <div class="tagcloud">
            <?php the_tags(null, ''); ?>
